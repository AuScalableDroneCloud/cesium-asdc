...
export class cropBoxMap {
  ...
  createSide = (planeLocal) => {
    ...
    var computeMoveAmount = (scene, position, direction, mouseMove) => {
  ...
  createEdge = (localEdge) => {
    ...
    const style = {
      ...
      highlightColor:
        Cesium.Color.fromCssColorString("rgb(91, 139, 81)").withAlpha(0.7),
    };
  ...
  disable() {
    this.clippingPlanes.enabled = false;
  ...
  constructor(translation, scale) {
    ...
    CORNER_POINT_VECTORS.map((vector, i) => {
      ...
      const downPoint = Cesium.Cartesian3.clone(
        upPoint,
        new Cesium.Cartesian3()
      );
      downPoint.z *= -1;
      ...
      const nextDownPoint = Cesium.Cartesian3.clone(
        nextUpPoint,
        new Cesium.Cartesian3()
      );
      nextDownPoint.z *= -1;
