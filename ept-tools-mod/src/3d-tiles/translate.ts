 ...
 */
export async function translate({ filename, cache, options = {}, cookie }: Translate) {
  ...
  const getOptions = {headers:{Cookie:cookie}};

  var ept =
    (await cache?.get(eptfilename,getOptions)) ||
    JsonSchema.validate<Ept>(Ept.schema, await getJson(eptfilename,getOptions))[0]
  ...
  if (extension === 'json') {
    ...
    const hierarchy = JsonSchema.validate<Hierarchy>(
      Hierarchy.schema,
      await getJson(join(eptdir, 'ept-hierarchy', `${Key.stringify(key)}.json`),getOptions)
    )[0]
